...
public:
    ...
    void build(int ind,int low,int high,vector<int>& arr){
        ...
        // For range min seg[ind] = min(seg[2*ind+1],seg[2*ind+2]);
    }
    ...
    void update(int ind,int low,int high,int l,int r,int val,vector<int>& arr){
        ...
        if(lazy[ind] != 0){
            seg[ind] += (high-low+1)* lazy[ind];    // This is for taking sum of all
            //seg[ind] += lazy[ind];
        ...
        if(low >= l && high <= r){
            ...
            // seg[ind] += val; For range min query   
    ...
    int query(int ind,int low,int high,int l,int r){
        ...
        if(lazy[ind] != 0){
            ...
            // seg[ind] += lazy[ind];  For range min query
...
int main(){
    ...
    while(q--){
        ...
        if(type == 1){
            ...
            cout<<st.query(0,0,n-1,l,r,val)<<endl;
        }
        else
        {
            int l,r,val;
